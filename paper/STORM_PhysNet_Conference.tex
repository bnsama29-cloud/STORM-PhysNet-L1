...
70/15/15\% with no shuffling.\footnote{The official tables use the released chronological cut (63\,394 / 7\,924 / 15\,850 hours). The preprocessor default 70/15/15 cut is not the cut used for those numbers.} All models share the same preprocessor, chronological windows, and storm-biased sampler; test PE is computed once after training and is never used for model selection. All models use the same storm-weighted sampler
(\(12\times\) oversampling of storm-flagged hours; see \texttt{config.yaml}).
...
STORM-Bz                     & \(0.986\) & \(0.900\) & \(0.854\) & \(0.812\) & \(\mathbf{+0.31}\) \\
