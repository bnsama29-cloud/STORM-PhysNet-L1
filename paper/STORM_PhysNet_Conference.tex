...
through LSTM- and MLP-based deep-learning approaches~\cite{b5,b6,b7}
...
characterized across seeds, and (ii) we are not aware of a published 15-seed
GOES\(\rightarrow\)GRASP hourly recipe for assets such as GSAT-19.
...
70/15/15\% with no shuffling.\footnote{The official tables use the released chronological cut (63\,394 / 7\,924 / 15\,850 hours). The preprocessor default 70/15/15 cut is not the cut used for those numbers.} All models share the same preprocessor, chronological windows, and storm-biased sampler; test PE is computed once after training and is never used for model selection.
...
reason zero-shot transfer fails and motivates
zero-padding, freezing the encoder, and fine-tuning the heads.
...
Solar-wind transit from L1 to the magnetopause is typically on the order of 0.5--1.5\,h~\cite{b3,b4} depending on solar-wind speed. Rather than a fixed offset,
...
Bagging fifteen STORM seeds (\(0.910\) at 6\,h) is stronger than the hybrid ensemble and provides a similar multi-horizon profile without requiring the Transformer at inference. We recommend bagging operationally due to this computational simplicity and superior performance.
...
STORM-Bz                     & \(\mathbf{0.986}\) & \(0.900\) & \(0.854\) & \(0.812\) & \(\mathbf{+0.31}\) \\
STORM (No-Delay)             & \(0.987\) & \(0.902\) & \(0.855\) & \(0.809\) & \(+0.36\) \\
...
\midrule
STORM-Bz bagged              & \(\mathbf{0.987}\) & \(\mathbf{0.910}\) & \(\mathbf{0.870}\) & \(0.836\) & --- \\
...
STORM variants lead at 1\,h; No-Delay is highest. At 6\,h and 12\,h the single models are close,
...
encoder and updating only the forecast heads) raises 6\,h PE from
...
(\(p = 1.66\times10^{-13}\), \(d = 7.00\)). Absolute GRASP PE remains below
...
Among systems evaluated here, seed-bagged STORM-Bz
...
available at
\url{https://github.com/bnsama29-cloud/STORM-PhysNet-L1}. Note that the GRASP fine-tuning run was executed externally, and its frozen outputs are provided in the repository.\footnote{The official tables use the released chronological cut
...
for forecasting GEO radiation belt electron fluxes,'' \emph{Space Weather}, vol.~22, e2024SW004119, 2024.
\bibitem{b10}
...
\emph{Space Weather}, vol.~16, pp.~1521--1548, 2018, doi:10.1029/2018SW002042.
\bibitem{b13}
