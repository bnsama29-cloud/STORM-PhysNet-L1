...
At \(\sigma=2\), Transformer PE\(_{1\mathrm{h}}\) falls from about
...
Robustness checks and GRASP fine-tuning support operational motivation beyond a pure leaderboard PE number; for instance, the architecture-matched baseline remains near persistence at 1\,h (PE$_{\mathrm{pers}}\approx-0.01$), whereas STORM-Bz is clearly positive (PE$_{\mathrm{pers}}\approx+0.31$). Furthermore, as a noise probe, under synthetic Gaussian noise injection (\(\sigma=1.0\)), the 6\,h PE of the matched baseline drops to \(0.825 \pm 0.011\), highlighting the need for structurally robust architectures during degraded sensor conditions.

Supplementary gating variants and a hybrid backbone were implemented
as probes and are not part of the primary conference comparison.
...
available at
\url{https://github.com/bnsama29-cloud/STORM-PhysNet-L1}. Note that the GRASP fine-tuning run and robustness/interpretability diagnostics were executed externally, and their frozen outputs are provided in the repository.
